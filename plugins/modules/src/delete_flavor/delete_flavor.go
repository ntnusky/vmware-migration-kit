...
func main() {
	...
	err = osm_os.DeleteFlavor(provider, moduleArgs.Name, moduleArgs.Cloud)
	if err != nil {
		response.Msg = "Failed to delete flavor: " + err.Error()
		FailJson(response)
	}

	response.Changed = true
	response.Msg = "Flavor deleted successfully"
	ExitJson(response)
}
