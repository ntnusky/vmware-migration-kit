...
func main() {
	...
	err = osm_os.DeleteVolume(provider, moduleArgs.Name, moduleArgs.Cloud)
	if err != nil {
		response.Msg = "Failed to delete volume: " + err.Error()
		FailJson(response)
	}

	response.Changed = true
	response.Msg = "Volume deleted successfully"
	ExitJson(response)
}
